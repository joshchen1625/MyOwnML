# -*- coding: utf-8 -*-
"""
Created on Mon May 13 22:56:52 2019
...
def stochasticGradientDescent(X, y, alpha = 0.000001, threshold = 0.01, loopNumLimit = 200):
    theta = np.zeros(X.shape[1], dtype = X.dtype)  # Init theta
    m = X.shape[0]  # Number of training examples
    n = X.shape[1]  # Number of features (including the intercept term)
    keepLooping = True
    ...
    while (iterNum <= loopNumLimit) and (keepLooping == True):
        print("----------------------", theta)
        s = np.zeros(X.shape[1], dtype = x.dtype)
...
updatedtheta = stochasticGradientDescent(X, y)
